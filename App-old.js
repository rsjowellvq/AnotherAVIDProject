...
const weFoundADevice = (peripheral) => {


  console.log("We Found A Device "+peripheral.id+" "+peripheral.name);
...
console.log("Cheeseburger 2007");
 ...
 setTimeout(()=>{
  ...
  BleManager.scan([],5400,false).then(()=>{
    ...
    for(var i=0;i<responseJson.data.length;i++)
    {
      if(responseJson.data[i].MinOfUseTotal > 0)
      {
        ...
        fetch("https://avid.vqconnect.io/nodejs/userList?action=findUserUsageDataByDay&dayTime="+responseJson.data[i]._id+"&uid="+currentUserData.uid+"&token="+currentUserData.token).then((responseaa)=>responseaa.json()).then((responseJsonaa)=>{
          ...
          console.log("Status is "+responseJsonaa.data[0].StandardTimeOfTreatment.substring(0,10));
          if(skipped==true)
          {
        ...
        console.log("DATA IS "+dataObject[responseJson.data[i]._id].dots)
  ...
  const About = ({navigation}) => {
    ...
    return(
      <View style={{alignItems: 'center',marginTop:'10%'}}>
        ...
        <Text>Username: {currentUserData.UserName}</Text>
